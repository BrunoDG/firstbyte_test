...
-   [ ] Start your project using (replace `project_name` with your project name and remove the curly braces):
    ```
    django-admin startproject firstbyte_test --extension py,json,yml,yaml,toml --name Dockerfile,README.md,.env.example,.gitignore,Makefile --template=https://github.com/vintasoftware/django-react-boilerplate/archive/refs/heads/main.zip
    ```
    ...
    django-admin startproject firstbyte_test . --extension py,json,yml,yaml,toml --name Dockerfile,README.md,.env.example,.gitignore,Makefile --template=https://github.com/vintasoftware/django-react-boilerplate/archive/refs/heads/main.zip
    ```
In the next steps, always remember to replace firstbyte_test with your project's name (in case it isn't yet):
-   [ ] Above: don't forget the `--extension` and `--name` params!
-   [ ] Go into project's root directory: `cd firstbyte_test`
-   [ ] Change the first line of README to the name of the project
-   [ ] Add an email address to the `ADMINS` settings variable in `firstbyte_test/backend/firstbyte_test/settings/base.py`
-   [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `firstbyte_test/backend/firstbyte_test/settings/production.py`
...
-   Do the following:
    -   Create a git-untracked `local.py` settings file:
        `cp backend/firstbyte_test/settings/local.py.example backend/firstbyte_test/settings/local.py`
    -   Create a git-untracked `.env.example` file:
        `cp backend/.env.example backend/.env`

### If you are using Docker:

-   Open the `backend/.env` file on a text editor and uncomment the line `DATABASE_URL=postgres://firstbyte_test:password@db:5432/firstbyte_test`
...
-   Open the `backend/.env` file on a text editor and do one of the following:
    ...
    -   If you wish to use PostgreSQL locally, uncomment and edit the line `DATABASE_URL=postgres://firstbyte_test:password@db:5432/firstbyte_test` in order to make it correctly point to your database URL
...
Chances are your project name isn't unique in Render, and you'll get a randomized suffix as your full app URL like: `https://firstbyte_test-a1b2.onrender.com`.

But this will only happen after the first deploy, so you are not able to properly fill `ALLOWED_HOSTS` yet. Simply set it to `*` then fix it later to something like `firstbyte_test-a1b2.onrender.com` and your domain name like `example.org`.
